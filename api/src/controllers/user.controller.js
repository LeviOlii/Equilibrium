...
export default async function buscarUsuarioPorId(req, res){
    try {
        const { id } = req.params;
...
export default async function atualizarUsuario(req, res){
    try {
        const { id } = req.params;
        const { nome, email, senha, tipo, Paciente, Profissional     } = req.body;
        console.log("Dados recebidos:", req.body);
...
export default async function deletarUsuario (req, res){
    try {
        const { id } = req.params;
